...
pub fn bounded_target(host: String, port: Int) -> Bool {
  ...
  && case string.split(host, ".") {
    ["100", second, third, fourth] ->
      case int.parse(second), int.parse(third), int.parse(fourth) {
        Ok(b), Ok(c), Ok(d) ->
          b >= 64 && b <= 127 && c >= 0 && c <= 255 && d >= 0 && d <= 255
        _, _, _ -> False
      }
    _ -> False
  }
...
/// Reuse the production framing boundary while retaining the connection for SSE.
pub fn handle(req: Request(Connection)) -> Response(ResponseData) {
  indrajaal_gleam_web.handle_bounded_connection_request(req, fn(_bounded) {
        ...
        |> response.set_body(
          mist.Bytes(
            bytes_tree.from_string(
              runtime_identity.to_json(runtime_identity.observe()),
            ),
          ),
        )
      Refused ->
        response.new(case req.method {
          http.Get -> 404
          _ -> 405
        })
        ...
        |> response.set_body(
          mist.Bytes(bytes_tree.from_string(
            "Manual testing only: read-only homeostasis routes; no file access or operational actions.",
          )),
        )
    }
  })
...
pub fn main() {
  ...
  let assert Ok(_) =
    mist.new(handle) |> mist.port(port) |> mist.bind(host) |> mist.start
  io.println(
    "Manual testing: http://nas-1.tail55d152.ts.net:"
    <> int.to_string(port)

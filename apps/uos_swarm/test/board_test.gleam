...
pub fn reply_to_lost_message_needs_a_causal_gap_record_test() {
  let lost = "1788762988041329-0613c673114ddcf5"
  let a =
    board.seal(
  ...
  let reply =
    board.seal(
      Draft(
        ..draft(board.Ack, "broadcast"),
        payload: [#("ack", lost)],
        causality: Causality(Some(lost), [lost]),
      ),
      "sw",
      2,
      2,
  ...
  let assert Error(e) = board.validate([signed(a), signed(reply)])
  e |> should.equal("reply to unknown message " <> lost)
  let gap =
    board.seal(
      Draft(..draft(board.Andon, "broadcast"), payload: [
        #("causal_gap", lost),
        #("reason", "wiped from the shared store before absorb"),
      ]),
      "sw",
      3,
      3,
  ...
  board.causal_gaps([gap]) |> should.equal([lost])

...
pub fn render(model: FmeaReportModel) -> String {
  let header =
    "\u{001b}[1;36m▌ FMEA Report\u{001b}[0m  Total RPN: "
    <> int_str(model.total_rpn)
    <> " | Critical: "
    <> int_str(model.critical_count)
  let table_hdr =
    "\u{001b}[90m  Mode                      S  O  D  RPN   Band      Mitigation\u{001b}[0m"
...
fn render_entry(e: FmeaEntry) -> String {
  ...
  <> "  "
  <> pad(e.mode, 26)
  <> pad(int_str(e.severity), 3)
  <> pad(int_str(e.occurrence), 3)
  <> pad(int_str(e.detection), 3)
  <> pad(int_str(e.rpn), 6)
  <> pad(fmea_report.rpn_band(e.rpn), 10)
  <> e.mitigation
  <> "\u{001b}[0m"
}

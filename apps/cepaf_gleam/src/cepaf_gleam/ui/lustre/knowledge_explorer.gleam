...
pub fn view(model: Model) -> Element(Msg) {
  ...
  html.div(
    [attribute.class("knowledge-explorer bg-slate-900 border border-slate-700 rounded-xl p-6 text-white shadow-2xl")],
    [
      render_header(),
      ...
      html.div([attribute.class("grid grid-cols-1 md:grid-cols-3 gap-6 mt-6")], [
        html.div([attribute.class("md:col-span-1 border-r border-slate-800 pr-4")], [
...
"  +-------------------------------------------------------------------------------------------------+
  ...
  html.div([attribute.class("my-3 bg-slate-950 border border-slate-800 rounded-lg p-3 font-mono text-[11px] overflow-x-auto")], [
    html.pre([attribute.class("text-amber-300 leading-tight select-all")], [html.text(ascii_art)]),
  ])
}
...
fn render_header() -> Element(Msg) {
  html.div([attribute.class("flex justify-between items-center mb-5 pb-4 border-b border-slate-800")], [
    html.div([], [
      ...
      html.p([attribute.class("text-xs text-slate-400 mt-1")], [
        html.text("Lustre MVU Living Knowledge Subsystem — Biosemiotic Transclusion Engine"),
      ]),
    ]),
    html.div([attribute.class("flex gap-2")], [
      html.span([attribute.class("px-2.5 py-1 text-xs font-mono rounded bg-emerald-500/20 text-emerald-400 border border-emerald-500/40")], [
        html.text("#rocha-semiotics"),
      ]),
      html.span([attribute.class("px-2.5 py-1 text-xs font-mono rounded bg-blue-500/20 text-blue-400 border border-blue-500/40")], [
        html.text("#cybernetics"),
      ]),
      html.span([attribute.class("px-2.5 py-1 text-xs font-mono rounded bg-amber-500/20 text-amber-400 border border-amber-500/40")], [
        html.text("#km-triad"),
      ]),
    ]),
  ])
...
fn render_tabs(active: KnowledgeTab) -> Element(Msg) {
  ...
  html.div([attribute.class("flex space-x-2 border-b border-slate-800 pb-2 mb-4 overflow-x-auto")], 
    list.map(tabs, fn(t) {
      let #(tab_val, label, icon) = t
      let is_active = tab_val == active
      let cls = case is_active {
        True -> "bg-amber-500/20 text-amber-300 border-b-2 border-amber-400 font-semibold"
        False -> "text-slate-400 hover:text-slate-200 hover:bg-slate-800/40"
      }
      html.button(
        [
          attribute.class("px-4 py-2 rounded-t text-xs flex items-center gap-2 transition-colors " <> cls),
...
fn render_search_bar(model: Model) -> Element(Msg) {
  html.div([attribute.class("mb-3")], [
    html.input([
      ...
      attribute.placeholder("Search articles, ADRs, invariants, transclusions..."),
...
fn render_tag_filter_strip(active_filter: Option(String)) -> Element(Msg) {
  let tags = ["#rocha-semiotics", "#cybernetics", "#km-triad", "#zero-muda", "#zk-adr"]

  html.div([attribute.class("flex items-center gap-2 text-xs flex-wrap mb-4")], [
    html.span([attribute.class("text-slate-500 text-[11px]")], [html.text("Filter tags:")]),
    html.button(
      [
        attribute.class("px-2 py-0.5 rounded text-[11px] font-mono border " <> case active_filter {
    ...
    ..list.map(tags, fn(t) {
      ...
      let cls = case is_selected {
        ...
        False -> "bg-slate-900/80 text-slate-400 border-slate-800 hover:text-amber-300 hover:border-slate-700"
      }
      html.button(
        [
          attribute.class("px-2 py-0.5 rounded text-[11px] font-mono border transition-all " <> cls),
...
fn render_items_list(items: List(KnowledgeItem), selected_id: Option(String)) -> Element(Msg) {
  case items {
    [] ->
      html.div([attribute.class("p-6 text-center text-slate-500 text-xs italic")], [
        html.text("No matching knowledge nodes found."),
      ])
    _ ->
      html.div([attribute.class("space-y-2 max-h-[420px] overflow-y-auto pr-1")], 
        list.map(items, fn(item) {
          ...
          html.div(
            [
              attribute.class("p-3 rounded-lg border cursor-pointer transition-all " <> card_cls),
              event.on_click(SelectItem(item.id)),
            ],
            [
              html.div([attribute.class("flex justify-between items-center mb-1")], [
                html.span([attribute.class("font-mono text-[11px] text-amber-400 font-semibold")], [html.text(item.id)]),
                html.span([attribute.class("font-mono text-[10px] text-slate-500")], [html.text(item.fractal_layer)]),
              ]),
              html.div([attribute.class("text-xs font-semibold text-slate-200 truncate")], [html.text(item.title)]),
              html.div([attribute.class("text-[11px] text-slate-400 line-clamp-2 mt-1")], [html.text(item.summary)]),
            ],
          )
        })
      )
...
fn render_inspector(items: List(KnowledgeItem), selected_id: Option(String)) -> Element(Msg) {
  ...
  case selected {
    None ->
      html.div([attribute.class("h-full flex items-center justify-center p-8 text-slate-500 text-xs italic")], [
        html.text("Select a knowledge node on the left to inspect its invariants and transclusions."),
      ])
    Some(item) ->
      html.div([attribute.class("bg-slate-950/80 border border-slate-800 rounded-lg p-5")], [
        html.div([attribute.class("flex justify-between items-start mb-3 pb-3 border-b border-slate-800")], [
          html.div([], [
            html.div([attribute.class("flex items-center gap-2 mb-1")], [
              html.span([attribute.class("px-2 py-0.5 rounded text-[10px] font-mono bg-amber-500/20 text-amber-300 border border-amber-500/40")], [
                html.text(item.id),
              ]),
              html.span([attribute.class("px-2 py-0.5 rounded text-[10px] font-mono bg-blue-500/20 text-blue-300 border border-blue-500/40")], [
                html.text(item.fractal_layer),
              ]),
            ]),
            html.h3([attribute.class("text-base font-bold text-slate-100")], [html.text(item.title)]),
          ]),
          html.a([
            attribute.href(item.tailscale_url),
            attribute.target("_blank"),
            attribute.class("px-3 py-1 text-xs rounded bg-blue-600 hover:bg-blue-500 text-white font-medium flex items-center gap-1"),
          ], [
            html.text("Open Full Doc ↗"),
          ]),
        ]),

        html.div([attribute.class("text-xs text-slate-300 leading-relaxed mb-4")], [
          html.text(item.summary),
        ]),

        html.div([attribute.class("mb-4")], [
          html.h4([attribute.class("text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-2")], [
            html.text("Bidirectional Transclusions"),
          ]),
          html.div([attribute.class("flex flex-wrap gap-2")], 
            list.map(item.transclusions, fn(tr) {
              html.span([attribute.class("px-2 py-1 rounded bg-slate-900 border border-slate-700 text-cyan-300 text-xs font-mono")], [
                html.text(tr),
              ])
            })
          ),
        ...
        html.div([], [
          ...
          html.div([attribute.class("flex flex-wrap gap-1.5")], 
            list.map(item.tags, fn(t) {
              html.span([attribute.class("px-2 py-0.5 rounded bg-slate-900/60 border border-slate-800 text-slate-400 text-[11px] font-mono")], [
                html.text(t),
              ])
            })
          ),

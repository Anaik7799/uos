...
pub fn view(model: Model) -> Element(Msg) {
  ...
  html.div([attribute.class("fpp-topology-container p-4 bg-slate-900 text-slate-200 rounded-lg space-y-6")], [
    render_header(),
    render_status_tiles(),
    render_instances_table(fpp_model.instances, fpp_model.components, model.selected_instance),
...
fn render_header() -> Element(Msg) {
  html.div([attribute.class("flex justify-between items-center pb-4 border-b border-slate-800")], [
    html.div([], [
      html.h2([attribute.class("text-2xl font-bold text-sky-400 flex items-center gap-2")], [
        html.span([], [html.text("🚀")]),
        html.text("NASA JPL F Prime / FPP Flight Software Topology"),
      ]),
      html.p([attribute.class("text-xs text-slate-400 mt-1")], [
        html.text("Pure BEAM / Gleam / OTP 29 Transmutation of Mission-Critical Aerospace Architecture"),
      ]),
    ]),
...
fn render_status_tiles() -> Element(Msg) {
  html.div([attribute.class("grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-3 text-center text-xs")], [
...
fn status_tile(value: String, label: String, color_class: String) -> Element(Msg) {
  ...
  html.div([attribute.class("bg-slate-950 p-4 rounded-lg border border-slate-800")], [
    ...
    html.div([attribute.class("overflow-x-auto")], [
      html.table([attribute.class("w-full text-left text-xs")], [
        ...
        html.tbody([], list.map(instances, fn(inst) {
          let comp_opt = list.find(components, fn(c) { c.comp_name == inst.of_component })
          let #(kind_str, ports_cnt, cmds_cnt, evts_cnt, chns_cnt) = case comp_opt {
            Ok(c) -> #(
              case c.kind {
                Passive -> "Passive"
                Queued -> "Queued"
          ...
          let row_class = case is_selected {
            ...
            False -> "border-b border-slate-800/40 hover:bg-slate-900/40 cursor-pointer"
          }
          html.tr([
            attribute.class(row_class),
            event.on_click(SelectInstance(inst.inst_name)),
          ], [
            html.td([attribute.class("p-2 font-mono text-sky-300")], [html.text(inst.inst_name)]),
            html.td([attribute.class("p-2 text-slate-300")], [html.text(inst.of_component)]),
            html.td([attribute.class("p-2")], [
              html.span([attribute.class("px-2 py-0.5 rounded text-[10px] font-mono bg-slate-800 text-slate-300")], [
                html.text(kind_str),
              ]),
            ]),
            ...
            html.td([attribute.class("p-2 font-mono text-slate-400")], [html.text(ports_cnt)]),
            html.td([attribute.class("p-2 font-mono text-indigo-400")], [html.text(cmds_cnt)]),
            html.td([attribute.class("p-2 font-mono text-emerald-400")], [html.text(evts_cnt)]),
            html.td([attribute.class("p-2 font-mono text-cyan-400")], [html.text(chns_cnt)]),
          ])
        })),
      ]),
    ]),
...
fn render_hsm_panel(active_state: String) -> Element(Msg) {
  html.div([attribute.class("bg-slate-950 p-4 rounded-lg border border-slate-800 space-y-3")], [
    html.div([attribute.class("flex justify-between items-center")], [
      ...
      html.div([attribute.class("text-xs font-mono text-purple-400 bg-purple-950/40 border border-purple-800/40 px-3 py-1 rounded-full")], [
        html.text("Active Path: " <> active_state),
      ]),
    ]),
    html.p([attribute.class("text-xs text-slate-400")], [
      html.text("Executes Lowest Common Ancestor (LCA) state exit and entry ordering, multi-level signal bubbling from leaf to root, and recursive initial sub-state resolution."),
    ]),
    html.div([attribute.class("grid grid-cols-1 md:grid-cols-3 gap-3 pt-2 text-xs font-mono")], [
      hsm_state_card("Operational", "Root State", "Sub-states: Flight, SurfaceOps", "Initial: Flight"),
      hsm_state_card("Flight", "Composite State", "Sub-states: Cruise, Landing", "Initial: Cruise"),
      hsm_state_card("SafeMode", "Recovery State", "Emergency safe mode on anomaly / solar storm", "Initial: None"),
    ]),
  ])
}

fn hsm_state_card(name: String, role: String, details: String, initial: String) -> Element(Msg) {
  html.div([attribute.class("bg-slate-900 p-3 rounded border border-slate-800 space-y-1")], [
    html.div([attribute.class("flex justify-between items-center")], [
      html.span([attribute.class("font-bold text-purple-300")], [html.text(name)]),
      html.span([attribute.class("text-[10px] text-slate-500")], [html.text(role)]),
    ]),
    html.p([attribute.class("text-[11px] text-slate-400")], [html.text(details)]),
    html.p([attribute.class("text-[10px] text-purple-400/80")], [html.text(initial)]),
  ])
}
...
fn render_subtopology_panel(model: domain.Model) -> Element(Msg) {
  html.div([attribute.class("bg-slate-950 p-4 rounded-lg border border-slate-800 space-y-3")], [
    ...
    html.div([attribute.class("space-y-2")], list.map(model.subtopologies, fn(sub) {
      html.div([attribute.class("bg-slate-900 p-3 rounded border border-slate-800 flex flex-col md:flex-row md:items-center md:justify-between gap-2")], [
        html.div([], [
          html.span([attribute.class("font-mono font-bold text-sky-300 text-xs")], [html.text(sub.name)]),
          html.span([attribute.class("text-slate-500 text-xs ml-2")], [
            html.text("(" <> int.to_string(list.length(sub.instances)) <> " internal instances, " <> int.to_string(list.length(sub.connections)) <> " internal connections)"),
          ]),
        ]),
        html.div([attribute.class("flex flex-wrap gap-2")], list.map(sub.exported_ports, fn(ep) {
          html.span([attribute.class("px-2 py-0.5 bg-slate-800 text-amber-300 text-[11px] font-mono rounded border border-slate-700")], [
            html.text(ep.name <> " ➔ " <> ep.instance_name <> "." <> ep.port_name),
          ])
        })),
      ])
    })),
...
fn render_links_panel() -> Element(Msg) {
  html.div([attribute.class("bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-3")], [
    html.div([], [
      html.h4([attribute.class("text-xs font-semibold text-slate-300 uppercase tracking-wider")], [
        html.text("Ground Station Integration"),
      ]),
      html.p([attribute.class("text-xs text-slate-500 mt-0.5")], [
        html.text("Download NASA JPL Ground Station Dictionary adhering to the official JSON schema specification."),
      ]),
    ]),
    html.a([
      attribute.href("/api/fpp/dictionary"),
      attribute.target("_blank"),
      attribute.class("px-4 py-2 bg-sky-600 hover:bg-sky-500 text-white font-mono text-xs font-bold rounded shadow transition-colors flex items-center gap-2"),
    ], [
      html.span([], [html.text("📥")]),

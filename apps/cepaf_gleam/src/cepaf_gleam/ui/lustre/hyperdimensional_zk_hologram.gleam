...
pub fn build_canonical_hologram() -> ZkHologramModel {
  let nodes = [
    ...
    HologramNode("ADR-003", "Dual Lattice STM", 0, 180.0, 220.0, 13.0, "#38bdf8"),
    ...
    HologramNode("ADR-007", "Zero-Trust Dispatch", 2, 420.0, 120.0, 13.0, "#f59e0b"),
    HologramNode("ADR-008", "Parsoid Roundtrip", 3, 490.0, 240.0, 14.0, "#ec4899"),
    HologramNode("ADR-009", "Linear Arenas", 1, 320.0, 340.0, 12.0, "#10b981"),
    HologramNode("ADR-010", "DMC Biosemiotics", 2, 460.0, 320.0, 15.0, "#f59e0b"),
    HologramNode("ADR-011", "Two-Key Security", 2, 540.0, 160.0, 13.0, "#f59e0b"),
    HologramNode("ADR-012", "Lyapunov Stability", 1, 260.0, 420.0, 14.0, "#10b981"),
    HologramNode("ADR-013", "Zenoh PubSub", 1, 380.0, 430.0, 13.0, "#10b981"),
    HologramNode("ADR-014", "MAX Python Isolation", 1, 170.0, 440.0, 12.0, "#10b981"),
    HologramNode("ADR-015", "Sheaf Boundary", 3, 580.0, 280.0, 14.0, "#ec4899"),
    HologramNode("ADR-016", "Rocha Cut Decouple", 2, 520.0, 400.0, 15.0, "#f59e0b"),
  ]
  ...
  ZkHologramModel(
    nodes: nodes,
    edges: edges,
    modularity_q: 0.785, // Louvain community modularity Q
...
pub fn render_zk_hologram_view(h: ZkHologramModel) -> Element(msg) {
  html.div([attribute.class("zk-hologram-container")], [
    html.h3([], [element.text("Hyperdimensional ZK Hologram & Living Knowledge Graph")]),
    html.div([attribute.class("hologram-metrics-strip")], [
      ...
      html.div([attribute.class("metric-pill")], [
        ...
        element.text(int.to_string(node_count(h)) <> " / " <> int.to_string(edge_count(h))),
      ]),
      html.div([attribute.class("metric-pill")], [
    ...
    html.div([attribute.class("hologram-graph-wrapper")], [
      html.div([attribute.class("graph-card")], [
        html.h4([], [element.text("Topological Knowledge Graph (Pure SVG Force Layout)")]),
        html.p([], [element.text("Clusters: Formal Logic (#38bdf8), Deterministic VFS (#10b981), Cybernetic Control (#f59e0b), Transclusion Sheaf (#ec4899)")]),
...
fn render_svg_hologram(h: ZkHologramModel) -> Element(msg) {
  ...
  let edge_elements = list.map(h.edges, fn(e) {
  ...
  let node_elements = list.map(h.nodes, fn(n) {
    element.element("g", [], [
      element.element(
        "circle",
        [
  ...
  element.element(
    ...
    [
      ...
      attribute.attribute("viewBox", "0 0 " <> int.to_string(svg_width) <> " " <> int.to_string(svg_height)),

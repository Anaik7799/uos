...
pub fn render_gospel_explorer_view(e: GospelExplorerState) -> Element(msg) {
  html.div([attribute.class("gospel-explorer-container")], [
    html.h3([], [element.text("Differential Gospel Contract & Bounded Z3 SMT Parity Explorer")]),
    html.div([attribute.class("gospel-metrics-bar")], [
      html.div([attribute.class("metric-pill")], [
        ...
        element.text(int.to_string(contract_count(e)) <> " / " <> int.to_string(contract_count(e))),
      ]),
      html.div([attribute.class("metric-pill")], [
      ...
      html.div([attribute.class("metric-pill")], [
        ...
        element.text(float.to_string(e.parsoid_roundtrip_fidelity *. 100.0) <> "% Exact Fidelity"),
      ]),
    ]),
    html.div([attribute.class("gospel-contracts-table-card"), attribute.attribute("style", "margin-top: 1rem; background: #161b22; padding: 1rem; border-radius: 6px; border: 1px solid #30363d;")], [
      html.h4([attribute.attribute("style", "margin-top: 0; color: #ffc107;")], [element.text("Gospel Specification Proof Ledger")]),
      html.table([attribute.attribute("style", "width: 100%; border-collapse: collapse;")], [
        html.thead([], [
          html.tr([], [
            html.th([attribute.attribute("style", "text-align: left; padding: 8px; color: #888;")], [element.text("Module & Symbol")]),
            html.th([attribute.attribute("style", "text-align: left; padding: 8px; color: #888;")], [element.text("Gospel Precondition")]),
            html.th([attribute.attribute("style", "text-align: left; padding: 8px; color: #888;")], [element.text("Gospel Postcondition")]),
            html.th([attribute.attribute("style", "text-align: left; padding: 8px; color: #888;")], [element.text("Z3 SMT Verdict")]),
            html.th([attribute.attribute("style", "text-align: left; padding: 8px; color: #888;")], [element.text("Solve Latency")]),
          ]),
        ]),
        html.tbody([], {
          use c <- list.map(e.contracts)
          html.tr([attribute.attribute("style", "border-top: 1px solid #222;")], [
            html.td([attribute.attribute("style", "padding: 8px; font-weight: bold; color: #58a6ff;")], [element.text(c.module_name <> " :: " <> c.function_symbol)]),
            html.td([attribute.attribute("style", "padding: 8px; font-family: monospace; font-size: 0.85rem;")], [element.text(c.precondition_spec)]),
            html.td([attribute.attribute("style", "padding: 8px; font-family: monospace; font-size: 0.85rem;")], [element.text(c.postcondition_spec)]),
            html.td([attribute.attribute("style", "padding: 8px; color: #10b981; font-weight: bold;")], [element.text(c.z3_result)]),
            html.td([attribute.attribute("style", "padding: 8px; color: #38bdf8;")], [element.text(float.to_string(c.solve_time_ms) <> " ms")]),
          ])
        }),
      ]),
    ]),

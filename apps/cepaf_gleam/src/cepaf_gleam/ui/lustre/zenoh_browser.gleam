...
pub type TopicNode {
  TopicNode(path: String, message_count: Int, children: List(TopicNode))
}
...
pub fn update(
  model: ZenohBrowserModel,
  msg: ZenohBrowserMsg,
) -> ZenohBrowserModel {
  case msg {
    TopicsLoaded(nodes) ->
      ZenohBrowserModel(..model, root: nodes, loading: False)
    SelectTopic(t) -> ZenohBrowserModel(..model, selected_topic: Some(t))
    MessageReceived(_topic, msg_text) ->
      ZenohBrowserModel(..model, last_message: msg_text)
    SubscribeTopic(t) ->
      ZenohBrowserModel(..model, subscribed: [t, ..model.subscribed])
    UnsubscribeTopic(t) ->
      ZenohBrowserModel(
        ..model,
        subscribed: list.filter(model.subscribed, fn(s) { s != t }),
      )
    RefreshTopics -> ZenohBrowserModel(..model, loading: True)
    ErrorReceived(e) ->
      ZenohBrowserModel(..model, error: Some(e), loading: False)
  }
}

pub fn total_topics(model: ZenohBrowserModel) -> Int {
  count_nodes(model.root)
}

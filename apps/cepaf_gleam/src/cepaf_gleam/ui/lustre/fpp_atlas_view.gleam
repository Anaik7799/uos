...
import cepaf_gleam/fpp/ontology.{category_to_string, derive_fpp_ontology}
...
pub type Model {
  Model(selected_tier: Option(algebraic_atlas.AtlasTier), active_tab: String)
}
...
pub fn init() -> Model {
  Model(selected_tier: Some(Tier1Topology), active_tab: "atlas")
}
...
pub fn view(model: Model) -> Element(Msg) {
  ...
  html.div(
    [
      attribute.class(
        "fpp-atlas-container p-4 bg-slate-900 text-slate-200 rounded-lg space-y-6",
      ),
    ],
    [
      render_header(),
    ...
    [
      html.div([], [
        html.h2(
          [
            attribute.class(
              "text-2xl font-bold text-amber-400 flex items-center gap-2",
            ),
          ],
          [
            html.span([], [html.text("🌌")]),
        ...
        html.p([attribute.class("text-xs text-slate-400 mt-1")], [
          html.text(
            "Category-Theoretic Topos, Sheaf Gluing & Biomorphic Semiotics on Pure BEAM / OTP 29",
          ),
        ]),
      ]),
...
fn render_metric_tiles(
  ...
  html.div(
    [
      attribute.class(
        "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3 text-center text-xs",
      ),
    ],
    [
      metric_tile("5 TIERS", "Category Hierarchy", "text-amber-400"),
      metric_tile(
        int.to_string(list.length(onto.nodes)),
        "Onto Nodes",
        "text-sky-400",
      ),
      metric_tile(
        int.to_string(list.length(onto.edges)),
        "Onto Edges",
        "text-indigo-400",
      ),
      metric_tile("7/7 DISJOINT", "DMC Windows", "text-emerald-400"),
      metric_tile(
        int.to_string(atlas.morphisms_count),
        "Morphisms",
        "text-purple-400",
      ),
...
fn render_navigation_tabs(active: String) -> Element(Msg) {
  html.div(
    [
      attribute.class(
        "flex gap-2 border-b border-slate-800 pb-2 text-xs font-mono",
      ),
    ],
    [
    ...
    html.div(
      ...
      [
        ...
        html.p([attribute.class("text-xs text-slate-400")], [
          html.text(
            "Functors map syntactically verified FPP ASTs into topological models, realized as supervised OTP 29 actors, observed as telemetry sheaves, and grounded in Rocha biosemiotics.",
          ),
        ]),
        html.div(
          [
            attribute.class(
              "grid grid-cols-1 md:grid-cols-5 gap-2 pt-2 text-xs font-mono",
            ),
          ],
          [
            tier_card(
            ...
            tier_card(
              "Tier 4: RochaSemiotic",
              "Biosemiotic Grounding",
              "Physical Actuators",
              "Dimension 4",
            ),
          ],
      ...
      [
        html.h3(
          ...
          [
            html.span([], [html.text("🔗")]),
            html.text(
              "Functorial Homomorphisms & Natural Transformations ("
              <> int.to_string(report.morphisms_count)
              <> ")",
            ),
          ],
        ),
            ...
            html.tbody(
              ...
              list.map(report.morphisms, fn(m) {
                html.tr(
                  [
                    attribute.class(
                      "border-b border-slate-800/40 hover:bg-slate-900/40 font-mono",
                    ),
                  ],
                  [
                    html.td([attribute.class("p-2 text-amber-300 font-bold")], [
                      html.text(m.name),
                    ]),
                    html.td([attribute.class("p-2 text-sky-400")], [
                      html.text(tier_to_string(m.source_tier)),
                    ]),
                    html.td([attribute.class("p-2 text-purple-400")], [
                      html.text(tier_to_string(m.target_tier)),
                    ]),
                    html.td(
                      [attribute.class("p-2 text-slate-400 text-[11px]")],
                      [html.text(m.source_id)],
                    ),
                    html.td(
                      [attribute.class("p-2 text-slate-300 text-[11px]")],
                      [html.text(m.target_id)],
                    ),
                    html.td([attribute.class("p-2 text-emerald-400")], [
                      html.text("PASS"),
                    ]),
                  ],
                )
...
fn tier_card(
  ...
  html.div(
    ...
    [
      html.div([attribute.class("font-bold text-amber-300 text-[11px]")], [
        html.text(title),
      ]),
      html.div([attribute.class("text-[10px] text-slate-400")], [
        html.text(subtitle),
      ]),
      html.div([attribute.class("text-[10px] text-slate-500 pt-1")], [
        html.text(desc),
      ]),
      html.div(
        [attribute.class("text-[9px] text-emerald-400/80 font-mono pt-1")],
        [html.text(dim)],
      ),
    ],
  )
      ...
      html.div([attribute.class("flex justify-between items-center")], [
        html.h3(
          ...
          [
            html.span([], [html.text("🧬")]),
            html.text(
              "Living Biomorphic Ontology Graph ("
              <> int.to_string(list.length(graph.nodes))
              <> " Nodes, "
              <> int.to_string(list.length(graph.edges))
              <> " Edges)",
            ),
          ],
        ),
        html.span(
          [
            attribute.class(
              "text-xs font-mono text-emerald-400 bg-emerald-950/40 border border-emerald-800/40 px-3 py-1 rounded-full",
            ),
          ],
          [
      ...
      html.div([attribute.class("overflow-x-auto max-h-[400px]")], [
        html.table([attribute.class("w-full text-left text-xs")], [
          html.thead(
            [
              attribute.class(
                "bg-slate-900/60 text-slate-400 border-b border-slate-800 font-mono sticky top-0",
              ),
            ],
            [
              html.tr([], [
          ...
          html.tbody(
            ...
            list.map(graph.nodes, fn(n) {
              html.tr(
                [
                  attribute.class(
                    "border-b border-slate-800/30 hover:bg-slate-900/40 font-mono text-[11px]",
                  ),
                ],
                [
                  html.td([attribute.class("p-2 text-sky-300")], [
                    html.text(n.id),
                  ]),
                  html.td([attribute.class("p-2 text-slate-300")], [
                    html.text(n.name),
                  ]),
                  html.td([attribute.class("p-2 text-amber-400")], [
                    html.text(category_to_string(n.category)),
                  ]),
                  html.td([attribute.class("p-2 text-slate-400")], [
                    html.text("L" <> int.to_string(n.layer)),
                  ]),
                  html.td([attribute.class("p-2 text-purple-300")], [
                    html.text(n.plane),
                  ]),
                  html.td([attribute.class("p-2 text-emerald-400")], [
                    html.text("ALIVE"),
  ...
  html.div([attribute.class("space-y-4")], [
    html.div(
      ...
      [
        ...
        html.p([attribute.class("text-xs text-slate-400")], [
          html.text(
            "Proves disjointness for every instance memory window: [BaseID, BaseID + Span) non-overlapping across the entire flight topology.",
          ),
        ]),
        html.div(
          [
            attribute.class(
              "grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-2 pt-2",
            ),
          ],
          list.map(dmc.windows, fn(w) {
            html.div(
              [
                attribute.class(
                  "bg-slate-900 p-3 rounded border border-slate-800 space-y-1 font-mono text-xs",
                ),
              ],
              [
                html.div([attribute.class("text-sky-300 font-bold")], [
                  html.text(w.instance_name),
                ]),
                html.div([attribute.class("text-slate-400 text-[10px]")], [
                  html.text("Component: " <> w.component_name),
                ]),
                html.div([attribute.class("text-amber-400 text-[11px]")], [
                  html.text(
                    "Range: [0x"
                    <> int.to_base16(w.base_id)
                    <> ", 0x"
                    <> int.to_base16(w.upper_bound)
                    <> ")",
                  ),
                ]),
                html.div([attribute.class("text-emerald-400 text-[10px]")], [
                  html.text("Span: " <> int.to_string(w.span) <> " IDs"),
                ]),
              ],
            )
          }),
    ...
    html.div(
      [
        attribute.class(
          "bg-slate-950 p-4 rounded-lg border border-rose-900/40 space-y-2",
        ),
      ],
      [
        html.h3(
          [
            attribute.class(
              "text-sm font-semibold text-rose-400 uppercase tracking-wider flex items-center gap-2",
            ),
          ],
          [
            html.span([], [html.text("🛑")]),
        ...
        html.p([attribute.class("text-xs text-slate-400")], [
          html.text(
            "Enforced across Kubernetes Rook-Ceph, Hermes OCaml Zero-Trust Dispatch, and Gleam Denotational Intent Router. All write, format, or mount operations on this device are unconditionally denied with HTTP 403 Forbidden.",
          ),
        ]),
      ],
    ),
...
fn render_api_links() -> Element(Msg) {
  html.div(
    [
      attribute.class(
        "bg-slate-950 p-4 rounded-lg border border-slate-800 flex flex-wrap justify-between items-center gap-3 text-xs font-mono",
      ),
    ],
    [
...
fn api_button(href: String, label: String) -> Element(Msg) {
  html.a(
    [
      ...
      attribute.class(
        "px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-sky-300 rounded border border-slate-700 transition-colors",
      ),
    ],
    [html.text(label)],
  )

...
pub type Selection {
  Selection(mode: Mode, scenario: Scenario, cycle: Int)
}

pub type Review {
  DeniedNoAuthority
  Preview(within_thresholds: Bool)
}
    ...
    && memory_limit <=. 1.0
  {
    ...
    True ->
      case selection.mode {
        ...
        TestData ->
          case selection.scenario {
            MissingSource -> Error("No simulated observation is available.")
            Disturbance ->
              Ok(Preview(0.98 <=. cpu_limit && 0.95 <=. memory_limit))
...
pub fn parse(query: List(#(String, String))) -> Result(Selection, String) {
  ...
  case mode, scenario, cycle {
    ...
    "test", "nominal", Ok(n) if n >= 1 && n <= 30 ->
      Ok(Selection(TestData, Nominal, n))
    "test", "disturbance", Ok(n) if n >= 1 && n <= 30 ->
      Ok(Selection(TestData, Disturbance, n))
    "test", "recovery", Ok(n) if n >= 1 && n <= 30 ->
      Ok(Selection(TestData, Recovery, n))
    "test", "unavailable", Ok(n) if n >= 1 && n <= 30 ->
      Ok(Selection(TestData, MissingSource, n))
    _, _, _ ->
      Error(
        "mode=real|test; scenario=nominal|disturbance|recovery|unavailable; cycle=1..30",
      )
  }
}
...
pub fn scenario_name(scenario: Scenario) -> String {
  case scenario {
    ...
    Recovery -> "recovery"
    MissingSource -> "unavailable"
  }
}

pub fn query(selection: Selection) -> String {
  "mode="
  <> mode_name(selection.mode)
  <> "&scenario="
  <> scenario_name(selection.scenario)
  <> "&cycle="
  <> int.to_string(selection.cycle)
}
...
pub fn read(selection: Selection) -> #(status.Snapshot, Int) {
  ...
  case selection.mode, selection.scenario {
    RealData, _ -> #(
      status.runtime_observation(beam_metrics.snapshot(), now),
      now,
    )
    ...
    TestData, scenario -> {
      ...
      let start_time = case scenario {
        Nominal -> selection.cycle * 10_000_000 + 5_000_000
        _ -> 0
      }
      ...
      let seed = case scenario == Nominal {
        True ->
          cycles(selection.cycle)
          |> list.fold(seed, fn(s, n) { evolve_model(s, n) |> result.unwrap(s) })
        False -> seed
      }
      let model =
        cycles(selection.cycle)
        |> list.fold(seed, fn(state, tick) {
          let tick_health = case scenario == Recovery && tick <= 2 {
            True -> 0.35
            False -> health
          }
          engine.ingest_telemetry(
            ...
            tick_health,
            1.0,
            start_time + tick * 1_000_000,
          )
        })
      let phys =
        physiology.update_physiological_telemetry(
          model.physiological,
          [
            #(physiology.CpuUtilization, cpu),
            #(physiology.MemoryUtilization, memory),
            #(physiology.RequestLatency, latency),
            #(physiology.ErrorRate, errors),
          ],
          1.0,
          model_time,
        )
      #(
        status.simulated(
          engine.HomeostasisSystemState(..model, physiological: phys),
        ),
        now,
      )
    }
...
pub fn evolve_model(
  state: engine.HomeostasisSystemState,
  cycle: Int,
) -> Result(engine.HomeostasisSystemState, String) {
  let now = cycle * 10_000_000
  let stable =
    cycles(3)
    |> list.fold(state, fn(s, tick) {
      engine.ingest_telemetry(s, 1.0, 1.0, now + tick * 1_000_000)
    })
  let mutation =
    engine.EvolutionaryMutation(
      "test-cycle-" <> int.to_string(cycle),
      "model-only",
      "Deterministic fixture evolution",
      1.0,
      0.0,
    )
  use submitted <- result.try(
    engine.submit_evolution(stable, mutation, now + 4_000_000)
    |> result.map_error(engine.evolution_error_to_string),
  )
  use voted <- result.try(
    [quorum.AgySovereign, quorum.ClaudeSovereign, quorum.CodexSovereign]
    |> list.try_fold(submitted, fn(pair, member) {
      engine.cast_registered_vote(
        pair.0,
        pair.1,
        member,
        quorum.QuorumApprove,
        "SIMULATED vote",
        "fixture-only",
        now + 5_000_000,
      )
  ...
  engine.apply_ratified_evolution(voted.0, voted.1)
  |> result.map_error(engine.evolution_error_to_string)
}

pub fn cycles(count: Int) -> List(Int) {
  list.repeat(Nil, int.clamp(count, 0, 30))

...
pub fn all_skills_and_superpowers() -> List(SkillCapability) {
  [
    SkillCapability("using-superpowers", "Superpower", "Establishes superpower skill invocation discipline before any response", ".agents/skills/using-superpowers/SKILL.md", 0.99),
    SkillCapability("lustre-gleam-ui-expert", "Skill", "Lustre 5.6+ MVU server-side rendered UI development without client JavaScript", ".agents/skills/lustre-gleam-ui-expert/SKILL.md", 0.98),
    SkillCapability("c3i-page-evolution", "Skill", "Comprehensive prompt covering all items from planning page evolution to full agentic UI", ".agents/skills/c3i-page-evolution/SKILL.md", 0.97),
    SkillCapability("ocaml-playwright-control", "Skill", "Automated headless browser control, screenshots, and DOM contracts via Playwright", ".agents/skills/ocaml-playwright-control/SKILL.md", 0.96),
    SkillCapability("chrome-devtools", "Skill", "Direct Chrome DevTools Protocol (CDP) control and DOM manipulation", ".agents/skills/chrome-devtools/SKILL.md", 0.95),
    SkillCapability("zk-knowledge-base", "Skill", "Zettelkasten knowledge base management, ADRs, MOCs, and bidirectional transclusion", ".agents/skills/zk-knowledge-base/SKILL.md", 0.98),
    SkillCapability("wiki-design", "Skill", "Hermes Wiki AST parsing, transclusion engine, and TyXML rendering", ".agents/skills/wiki-design/SKILL.md", 0.97),
    SkillCapability("living-ontology", "Skill", "Living ontology, STAMP/STPA safety lattices, and 13D trace coordinate tracking", ".agents/skills/living-ontology/SKILL.md", 0.99),
    SkillCapability("infranodus-design-superset", "Skill", "Network text analysis, cognitive discourse parsing, and hypergraph visualization", ".agents/skills/infranodus-design-superset/SKILL.md", 0.96),
    SkillCapability("formal-verification-pipeline", "Skill", "Lean 4 mathematical proofs, Quint simulation, and Gospel contract verification", ".agents/skills/formal-verification-pipeline/SKILL.md", 0.99),
    SkillCapability("writing-gospel-specifications", "Skill", "Gospel contract authoring for OCaml/BEAM boundary interfaces", ".agents/skills/writing-gospel-specifications/SKILL.md", 0.95),
    SkillCapability("algebraic-fractal-structures", "Skill", "Fractal multi-scale system architecture and 4-tensor product mappings", ".agents/skills/algebraic-fractal-structures/SKILL.md", 0.98),
    SkillCapability("systematic-debugging", "Skill", "Rigorous 4-phase debugging before proposing code changes", ".agents/skills/systematic-debugging/SKILL.md", 0.97),
    SkillCapability("test-driven-development", "Skill", "Red-Green-Refactor test-first development discipline", ".agents/skills/test-driven-development/SKILL.md", 0.98),
    SkillCapability("timestamp-sync", "Skill", "Host clock synchronization receipt verification and YYYYMMDD-HHSS- prefix mandate", ".agents/skills/timestamp-sync/SKILL.md", 0.99),
    SkillCapability("patrol-marionette-test", "Skill", "Mobile-first and browser test automation for multi-screen workflows", ".agents/skills/patrol-marionette-test/SKILL.md", 0.94),
  ]
}
...
pub fn all_standards_and_algorithms() -> List(AlgorithmStandard) {
  [
    ...
    AlgorithmStandard("Google", "Core Web Vitals (CWV)", "W3C / Google Web Vitals Spec", "Performance & UX Metric", "LCP <= 2.5s, INP <= 200ms, CLS <= 0.1", "Enforces sub-second page loads, zero client-JS latency, and visual stability in Lustre pages", 0.98, True),
    AlgorithmStandard("Google", "PageRank & Personalized PageRank (PPR)", "Page et al. (1998), Haveliwala (2002)", "Graph Random Walk with Restart", "PR(u) = (1-d)/N + d * sum(PR(v)/L(v)), d = 0.85", "Used in Hermes/ZigVM knowledge graph to rank ADRs and notes by structural importance and topic relevance", 0.97, True),
    AlgorithmStandard("Google", "HITS (Hubs & Authorities)", "Kleinberg (1999)", "Mutually Recursive Link Analysis", "h(p) = sum(a(q)), a(p) = sum(h(q))", "Distinguishes between index MOCs (Hubs) and authoritative technical specifications (Authorities)", 0.96, True),
    AlgorithmStandard("Google", "SimHash & MinHash (LSH)", "Charikar (2002), Broder (1997)", "Locality-Sensitive Hashing", "Hamming distance on 64/128-bit hash vectors", "Detects duplicate and near-duplicate documentation and journal entries across corpora", 0.95, True),
    AlgorithmStandard("Google", "BM25 / BM25F", "Robertson et al. (1994)", "Probabilistic Information Retrieval", "IDF * (f * (k1 + 1)) / (f + k1 * (1 - b + b * (|D|/avgdl)))", "Powers lexical search ranking in the knowledge base across title, tags, and body fields", 0.96, True),
    AlgorithmStandard("Google", "W3C OpenTelemetry (OTel)", "W3C Distributed Tracing Recommendation", "Observability Protocol", "128-bit trace_id, 64-bit span_id, microsecond UTC timestamps", "Propagates distributed trace context across all 15 UI pages over Zenoh pub/sub mesh", 0.99, True),
    AlgorithmStandard("Google", "Lighthouse CI / WCAG 2.1 AA", "W3C Web Content Accessibility Guidelines", "Automated Accessibility Audit", "Contrast ratio >= 4.5:1, aria-labels, semantic landmarks", "Verified by Playwright and Wallaby browser test suites across all 31 C3I pages", 0.95, True),
    AlgorithmStandard("Google", "Schema.org & JSON-LD", "W3C RDF in JSON Recommendation", "Linked Data Semantic Markup", "JSON-LD context graphs with @type and @id", "Embedded in Lustre web headers for semantic machine-readability of system specs", 0.94, True),

    // --- Wikipedia / MediaWiki Standards & Algorithms (5) ---
    AlgorithmStandard("Wikipedia/MediaWiki", "Aho-Corasick Multi-Pattern Automaton", "Aho & Corasick (1975)", "Deterministic Finite Automaton", "Linear time O(n + m) multi-string matching", "Scans markdown text to automatically identify unlinked note titles and suggest backlinks", 0.98, True),
    AlgorithmStandard("Wikipedia/MediaWiki", "Myers Diff & Patience Diff", "Myers (1986), Bram Cohen (2006)", "Shortest Edit Script & LCS", "O(ND) search over edit graph", "Generates human-readable, semantic visual diffs between document revisions in dual view mode", 0.97, True),
    AlgorithmStandard("Wikipedia/MediaWiki", "Transclusion Engine & Cycle Guard", "MediaWiki Template Transclusion Spec", "Recursive AST Expansion with SCC Detection", "Tarjan SCC algorithm, max_recursion_depth <= 16", "Enables [[wiki:...]] and [[zk:...]] transclusion while permanently preventing infinite cyclic loops", 0.99, True),
    AlgorithmStandard("Wikipedia/MediaWiki", "Parsoid Round-Trip Fidelity", "MediaWiki Parsoid AST Spec", "Bidirectional AST Serialization", "Markdown AST <-> HTML5 without lossy conversion", "Preserves exact document structure and block anchors (^id) across editing cycles", 0.96, True),
    AlgorithmStandard("Wikipedia/MediaWiki", "Category Sheaf & DAG Invariants", "MediaWiki Category Invariant Spec", "Acyclic Directed Graph Verification", "DFS back-edge detection: G must have no directed cycles", "Guarantees that the topic taxonomy and MOC hierarchy form a well-founded poset", 0.97, True),

    // --- Zettelkasten & Second Brain Standards & Algorithms (6) ---
    AlgorithmStandard("Zettelkasten/Obsidian", "Adjacency Matrix Inversion", "Graph Theory", "Backlink Indexing Algorithm", "A^T = Invert(A) where A_ij = link(i, j)", "Automatically computes bidirectional backlinks for every ZK note and ADR in O(|E|) time", 0.99, True),
    AlgorithmStandard("Zettelkasten/Obsidian", "Obsidian Block Anchors (^id)", "Obsidian Spec / ZK Slipbox Standard", "Surgical Addressing Specification", "Anchor pattern: ^[a-zA-Z0-9_-]+$", "Enables surgical transclusion and linking to individual paragraphs, tables, or callouts", 0.98, True),
    AlgorithmStandard("Zettelkasten/Obsidian", "Louvain & Leiden Community Detection", "Blondel et al. (2008), Traag et al. (2019)", "Modularity Optimization", "Q = 1/(2m) * sum(A_ij - k_i*k_j/(2m)) * delta(c_i, c_j)", "Discovers emergent topic clusters and semantic modules in the knowledge graph", 0.95, True),
    AlgorithmStandard("Zettelkasten/Obsidian", "Vector Cosine Distance Retrieval", "Information Retrieval / Vector Spaces", "Semantic Similarity Metric", "cos(theta) = (u . v) / (||u|| * ||v||)", "Computes similarity between ADRs and documents for related note suggestions", 0.96, True),
    AlgorithmStandard("Zettelkasten/Obsidian", "Dung Abstract Argumentation", "Dung (1995)", "Formal Logic & Argument Networks", "Lattice of admissible, preferred, and grounded extensions", "Validates architectural consistency and non-contradiction between permanent ADRs", 0.98, True),
    AlgorithmStandard("Zettelkasten/Obsidian", "Rocha Biosemiotics Symbol-Matter Cut", "Rocha (1998), Pattee (1982)", "Cybernetic Semiotic Architecture", "Triadic sign: Signifier (Token) -> Signified (State) -> Interpretant (Action)", "Enforces the formal distinction between pure symbolic code and physical actuators", 0.99, True),
  ]
}
...
pub fn verify_skills_effectiveness() -> #(Int, Float) {
  ...
  let sum_eff = list.fold(skills, 0.0, fn(acc, s) { acc +. s.effectiveness_score })
  let mean_eff = case total > 0 {
    True -> sum_eff /. int.to_float(total)
    False -> 0.0
  }

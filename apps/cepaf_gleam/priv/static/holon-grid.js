...
(function () {
  ...
  function updateHeartbeat() {
    var dot = document.getElementById("holon-heartbeat");
    if (!dot) return;
  ...
  function checkStaleness() {
    var elapsed = Date.now() - lastMsgTime;
    var banner = document.getElementById("holon-stale-banner");
    if (!banner) return;
  ...
  style.textContent = [
    "#holon-heartbeat{display:inline-block;width:9px;height:9px;border-radius:50%;background:#3dd68c;transition:background 0.3s;vertical-align:middle;margin-right:4px}",
    "#holon-stale-banner{display:none;background:#ff475722;border:1px solid #ff4757;border-radius:6px;padding:6px 12px;color:#ff4757;font-size:0.82rem;margin:8px 0}"
  ].join("");

...
pub fn pairwise_section_agreement_test() {
  let s1 =
    LocalSection(
      ...
      shared_state_digest: "sha256-state-alpha",
    )
  let s2 =
    LocalSection(
      page_route: "/settings",
...
pub fn pairwise_section_disagreement_test() {
  ...
  let s2 =
    LocalSection(
      page_route: "/telemetry",
      shared_state_digest: "sha256-state-beta",
    )
  check_pairwise_agreement(s1, s2)
  |> should.be_false
}
...
pub fn glue_consistent_sections_test() {
  let sections = [
    LocalSection(
      page_route: "/dashboard",
      shared_state_digest: "canonical-hash-xyz",
    ),
    LocalSection(
      page_route: "/telemetry",
      shared_state_digest: "canonical-hash-xyz",
...
pub fn glue_inconsistent_sections_test() {
  let sections = [
    ...
    LocalSection(
      page_route: "/telemetry",
      shared_state_digest: "divergent-hash-123",
    ),
  ]
  glue_sections(sections)

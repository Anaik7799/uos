...
import cepaf_gleam/verification/dmc_tcm_algebraic_atlas.{
  AtlasSheafSection, DmcSyntax, IntentSpec, Tcm13D,
  all_atlas_layers, all_atlas_morphisms, all_master_test_categories,
  check_clock_drift,
...
pub fn tcm_clock_drift_thresholds_test() {
  ...
  check_clock_drift(12000.0) |> should.be_error
}
...
pub fn algebraic_atlas_sheaf_gluing_test() {
  let sections = [
    ...
    AtlasSheafSection("SEC-02", [1, 2, 3], "Kernel-Component-Ledger Subsheaf", True),
    AtlasSheafSection("SEC-03", [3, 4, 5], "State-System-Cognitive Subsheaf", True),
    AtlasSheafSection("SEC-04", [5, 6, 7], "Cognitive-Mesh-Federation Subsheaf", True),
  ]
...
pub fn denotational_intent_storage_lock_gate_test() {
  ...
  let malicious_intent = IntentSpec(
    id: "INT-MAL-01",
    layer: 4,
...
pub fn denotational_intent_zero_trust_interception_test() {
  ...
  string.contains(nul_denotation.rejection_reason, "G-ZERO-TRUST-NUL") |> should.be_true

  // Test SQL injection
  let sqli_intent = IntentSpec(
    id: "INT-SQLI-01",
    layer: 3,
    actor_id: "agent-y",
  ...
  string.contains(sqli_denotation.rejection_reason, "G-ZERO-TRUST-SQLI") |> should.be_true
}

pub fn denotational_intent_authorized_execution_test() {
  let valid_intent = IntentSpec(
    id: "INT-VAL-01",
    layer: 5,
...
pub fn denotational_intent_api_json_serialization_test() {
  ...
  string.contains(json_str, "\"contract\":\"DMC-TCM-MANDATE\"") |> should.be_true
  string.contains(json_str, "\"status\":\"admitted\"") |> should.be_true
  string.contains(json_str, "http://nas-1.tail55d152.ts.net:4100") |> should.be_true
}
...
pub fn master_test_inventory_completeness_test() {
  ...
  should.be_true(total_tests >= 10600)
  ...
  string.contains(json_inventory, "\"contract\":\"UOS-CANONICAL-TEST-INVENTORY\"") |> should.be_true

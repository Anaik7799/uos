...
import cepaf_gleam/bridge/pi_startup_classifier.{
  StagePreflight, StageProviderAuth, StageProcessSpawn,
  StageProtocolHandshake, StageToolFederation, StageMeshSync,
  StageOperationalReady, StageFailed,
  nominal_stages, stage_to_id, stage_metadata, format_intelligent_message,
  stage_to_json, to_agui_event, init_startup, advance_stage, fail_startup,
  render_html_startup_card, render_ansi_startup_banner,
}
...
pub fn stage_metadata_calculation_test() {
  let meta1 = stage_metadata(StagePreflight, 50, "anthropic", "claude-3-5-sonnet")
  ...
  let meta_ready = stage_metadata(StageOperationalReady, 980, "deepseek", "deepseek-coder")
  ...
  let meta_failed = stage_metadata(
...
pub fn format_intelligent_message_test() {
  let meta_stage = stage_metadata(StageToolFederation, 420, "anthropic", "claude-3-7-sonnet")
  ...
  let meta_ready = stage_metadata(StageOperationalReady, 820, "anthropic", "claude-3-7-sonnet")
  let ready_msg = format_intelligent_message(meta_ready)
  string.contains(ready_msg, "Operational & Ready (820ms total)") |> should.equal(True)
...
pub fn state_machine_transitions_test() {
  let s0 = init_startup("anthropic", "claude-3-7-sonnet", 1_000)
  ...
  let s1 = advance_stage(s0, StageProviderAuth, 1_120)
  ...
  let s2 = advance_stage(s1, StageProcessSpawn, 1_250)
  ...
  let s_fail = fail_startup(s2, "Spawn failed: command not found", 1_300)

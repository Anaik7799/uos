...
fn refs(value: String) -> List(String) {
  case value { "-" -> [] _ -> string.split(value, ",") }
}
...
fn respond(outcome: Result(String, String)) -> Nil {
  case outcome {
    ...
    Error(error) -> {
      io.println(json.to_string(json.object([#("ok", json.bool(False)), #("error", json.string(error))])))
      halt(1)
    }
  }
}
...
pub fn run(args: List(String)) -> Result(String, String) {
  case args {
    [root, "register", s, provider, workspace, revision, metadata, op] ->
      sync.execute(root, sync.Register(s, provider, workspace, revision, refs(metadata)), op)
    ...
    [root, "renew", s, resource, epoch, ttl, op] -> {
      ...
      use seconds <- result.try(number(ttl))
      sync.execute(root, sync.Renew(s, resource, epoch, seconds * 1_000_000), op)
    }
    ...
    [root, "ack", s, message_id, op] -> sync.execute(root, sync.Ack(s, message_id), op)
    [root, "retire", s, op] -> sync.execute(root, sync.Retire(s), op)
    [root, "status"] -> sync.observe(root, fn(state, boot, now) { Ok(sync.status_json(state, boot, now)) })
    [root, "inbox", s] -> sync.observe(root, fn(state, _, _) { Ok(sync.inbox_json(state, s)) })
    [root, "check", s, resource, epoch] -> {
      ...
      sync.observe(root, fn(state, boot, now) {
        ...
        Ok(json.object([#("ok", json.bool(True)), #("resource", json.string(resource)),
          #("holder", json.string(s)), #("epoch", json.int(epoch)),
          #("checked_boot_us", json.int(now)), #("scope", json.string("cooperative fence observation; no action executed"))]))
      })
    }
    [root, "journal"] -> sync.read_journal(root)
    [root, "recover-lock"] -> {
      use message <- result.try(sync.recover_lock(root))
      Ok(json.to_string(json.object([#("ok", json.bool(True)), #("result", json.string(message))])))
    }
    _ -> Error(usage())
  }
}
...
pub fn main() -> Nil {
  ...
  case args { ["--help"] | ["help"] | [] -> io.println(usage()) _ -> respond(run(args)) }
}

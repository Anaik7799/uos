...
let write_ledger_atomic path value =
  ...
  if Filename.dirname path <> "/tmp" || basename = "" || basename = "."
     || basename = ".." || has_nul path || String.length value > max_ledger_bytes then
    ...
    try
      ...
      Fun.protect
        ...
        (fun () ->
          ...
          (* Publish without replacing any existing file or following a target
             symlink. Direct /tmp children exclude traversal and parent links. *)

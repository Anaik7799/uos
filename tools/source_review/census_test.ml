...
let () =
  ...
  Fun.protect
    ...
    (fun () ->
      ...
      check (Result.is_error (write_ledger_atomic ledger_path "replacement"))
        "publication must not overwrite an existing artifact";
      check (expect_ok (read_regular_bounded ~limit:max_ledger_bytes ledger_path) = observed)
        "rejected overwrite must preserve the exact existing artifact bytes");
  check (Result.is_error (write_ledger_atomic "/tmp/../tmp/uos-census-traversal-control" "x"))
    "parent traversal must be rejected before any output creation";
  let linked_parent = Filename.temp_file "uos-census-parent-" "" in
  Unix.unlink linked_parent;
  Unix.symlink "/tmp" linked_parent;
  Fun.protect ~finally:(fun () -> Unix.unlink linked_parent) (fun () ->
    check (Result.is_error (write_ledger_atomic (linked_parent ^ "/uos-census-parent-control") "x"))
      "symlinked parent must be outside the output namespace");

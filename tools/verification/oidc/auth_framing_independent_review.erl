...
%% Only owned ephemeral Tailnet FQDN sockets are used. No live service is contacted.
...
run() ->
  ...
  io:put_chars(json:encode(#{schema => <<"uos-auth-framing-independent/v1">>,
    ...
    network_scope => <<"owned nas-1.tail55d152.ts.net ephemeral sockets only">>, results => Results})),
  io:nl(),
...
with_socket(Fun) ->
  ...
  {ok,L}=gen_tcp:listen(0,[binary,{active,false},{ip,Address}]),
  try
    {ok,{Address,Port}}=inet:sockname(L),
    {ok,C}=gen_tcp:connect(Host,Port,[binary,{active,false}],1000),
    try

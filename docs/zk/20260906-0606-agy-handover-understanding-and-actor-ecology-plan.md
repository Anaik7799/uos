...
| UOS-H10 / P0-P1 | Pin and complete the C3I-derived Zenoh NIF, unify MoZ/native call paths, implement supervised sessions and map every UOS domain edge to Zenoh | H01,H09 | Actual native load and send/receive/query/queryable receipts; no dummy success; close/reopen, callback death, cancellation and scheduler tests; complete edge migration matrix |
...
Priority reflects defect risk and dependency, not a claim that all P1 tasks are small. Full SysML and FPP conformance are substantial implementation programmes. Stage their code delivery without reducing the operator's final requirement. Time estimates require completing the feature/clause census; no speculative deadline is asserted here.

...
```mermaid
graph TD
    subgraph H6["Holon 6: Cakra-Sañcaraṇa (चक्र-संचरण) - Mesh Telemetry"]
        Z1["Zenoh Pub/Sub Mesh (indrajaal/**)"]
        Z2["128-Bit W3C OTel Distributed Spans"]
    end

    subgraph H5["Holon 5: Pratyakṣa-Rakṣā (प्रत्यक्ष-रक्षा) - Zero-Trust Defense"]
        S1["Gleam Tri-Agent Monitor (tri_agent_monitor.gleam)"]
        S2["Hermes Zero-Trust Hook (run_agent_dispatch_hook.exe)"]
        S3["Sa-Plan Jidoka Fence (SC-JIDOKA-001)"]
    end

    subgraph H4["Holon 4: Prāṇa-Vyūha (प्राण-व्यूह) - Supervisory Life-Pulse"]
        G1["Pure Gleam / OTP 29 Supervisor (uos_sup)"]
        G2["Fast OODA Observe-Orient-Decide-Act Ring"]
        G3["Prajna Circuit Breakers (H_C >= 0.85)"]
        G4["Defense Threat Assessment (DEFCON 1-4)"]
        G5["Lyapunov Trend Detector & Health Derivative"]
    end

    subgraph H3["Holon 3: Pramāṇa-Viveka (प्रमाण-विवेक) - Formal Evidence"]
        V1["Hermes OCaml Engine"]
        V2["Gospel Formal Contracts & Z3 Solver"]
        V3["Rete-UL Forward-Chaining Rules"]
        V4["SQLite WAL Append-Only Ledgers"]
    end

    subgraph H2["Holon 2: Kevala-Kośa (केवल-कोश) - Unikernel Enclaves"]
        U1["MirageOS Library OS"]
        U2["Solo5 Tender Sandboxes"]
        U3["Microsecond Cold Boot (< 10 ms)"]
    end

    subgraph H1["Holon 1: Rasa-Dhātu (रस-धातु) - SIMD Neural Acceleration"]
        M1["Modular MAX / Mojo Kernel"]
    ...
    subgraph H0["Holon 0: Vajra-Adhiṣṭhāna (वज्र-अधिष्ठान) - Deterministic Kernel"]
        K1["ZigVM Pure Zig Engine"]
        K2["Descriptor-Relative Race-Free VFS"]
        K3["Linear Allocation Arenas (Zero GC)"]
        K4["Fuel-Bounded Process Ports (os_port.zig)"]
        K5["MAX Fabric Controller (max_fabric.zig)"]
    end

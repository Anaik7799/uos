...
**Closure (HOLON-LIFECYCLE, 2026-09-07 16:4x UTC, worker W-K `okyntnlylzzn@01138cca`).** Consequence 1 is fixed: an L0 `constitution` holon (`saṃvidhāna`, plane Control, whole `uos`, module `fractal/l0_constitutional.gleam`) is now the whole of the nine L0 constitutional process holons (`c3i-iam-native-guard-service`, `uos-clock-guard-service`, `iam-supervisor-gleam`, `vault-supervisor-gleam`, `fractal-l0-constitutional-gleam`, `ferriskey-vendored-operator-rust`, `rusty-vault-vendored-rust-source-of-rusty-vault-nif-so`, `ferriskey-nif`, `rusty-vault-nif`); `cepaf-gleam` and `uos-swarm` return to L2, `native-nifs` sits at L1 with its NIF children, and the seven plane holons plus `planes` are back at L1. Distribution: L0 12, L1 24, L2 24, L3 7, L4 58, L5 26, L6 6, L7 1 over 158 holons. The lifecycle is now a state machine (`transition(Lifecycle, LifecycleEvent)`: Dormant→Awakening→Active→Stressed→Healing→Active, any non-terminal state→Apoptotic, everything else an error) and is DERIVED from census status at census time (running→Active, stopped/absent→Dormant, failed→Stressed, superseded→Apoptotic), never observed live; `Vitals` (heartbeat age, restarts, last transition) is an optional field until a monitor feeds it. Consequence 2 (per-holon glosses) stays with KM.

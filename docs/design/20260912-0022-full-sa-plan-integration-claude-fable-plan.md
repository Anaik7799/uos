...
**Document Identifier**: `docs/design/20260912-0022-full-sa-plan-integration-claude-fable-plan.md`  
**Mandatory Timestamp Prefix**: `20260912-0022-` (`contracts/rules/timestamp-mandate.md`)  
...
**Canonical Sa-Plan Registration**: `uos/sa-plan-full/20260912-0022` in [`var/sa-plan/uos.sqlite3`](file:///home/an/NAS-setup/uos/var/sa-plan/uos.sqlite3)  
**Tailscale Web FQDN Link**: [http://nas-1.tail55d152.ts.net:8100/docs/design/20260912-0022-full-sa-plan-integration-claude-fable-plan.md](http://nas-1.tail55d152.ts.net:8100/docs/design/20260912-0022-full-sa-plan-integration-claude-fable-plan.md)  
...
### 4.3 High-Fidelity Simulator Architecture (`SC-SIM-001`)
```text
+---------------------------------------------------------------------------------------------------+
|                           SA-PLAN HIGH-FIDELITY SIMULATOR ARCHITECTURE                            |
+---------------------------------------------------------------------------------------------------+
|                                                                                                   |
|  [15-Worker Concurrent Claim Simulator]                                                           |
|   - 15 Simulated Workers (Fable, Codex, AGY, Pi, Gemma) concurrent pull requests                 |
|   - Verifies: Exactly 1 worker wins lease; 14 workers receive locked state (Zero Double-Claim)    |
|                                                                                                   |
|  [Zombie Lease Expiration & Reaper Simulator]                                                     |
|   - Injects simulated worker process crash mid-task (SIGKILL)                                     |
|   - Advances virtual clock past lease_until_ns                                                    |
|   - Verifies: Background reaper returns task to 'available' with incremented attempt counter      |
|                                                                                                   |
|  [Temporal Workflow Event-Sourced Replay Simulator]                                               |
|   - Loads historical sa_plan_workflow_event logs                                                  |
|   - Replays activities deterministically from genesis to terminal state                           |
|   - Verifies: Output state hash matches original execution receipt bit-for-bit                   |
|                                                                                                   |
|  [Hardware OS Drive Safety Violation Attack Simulator]                                           |
|   - Injects malicious payload targeting HARD_DENIED_SYSTEM_OS_SERIAL = "25503L801736"            |
|   - Verifies: Rust NIF & Gleam guard reject request; Andon stop line triggered (Exit -32002)     |
+---------------------------------------------------------------------------------------------------+
```
...
## 7. Comprehensive 9-Modality Test & Simulator Protocol

1. **Unit Testing**: Isolated verification of state transitions, lease calculations, and JSON codecs.
2. **System Integration Testing**: End-to-end plan creation $\to$ task claim $\to$ SIMD rank $\to$ execution $\to$ completion.
...
9. **High-Fidelity Simulators**:
   - *Worker Claim Simulator*: 15 concurrent synthetic workers pulling tasks simultaneously.
   - *Zombie Lease Reaper Simulator*: Simulated worker crashes, clock advances, and automatic task reclamation.
   - *Temporal Replay Simulator*: Deterministic activity log replay matching bit-for-bit hashes.
   - *Hardware Attack Simulator*: Malicious drive serial modification attempts fail-closed immediately.
...
- **In-Tree Sa-Plan Full Integration Plan**: [http://nas-1.tail55d152.ts.net:8100/docs/design/20260912-0022-full-sa-plan-integration-claude-fable-plan.md](http://nas-1.tail55d152.ts.net:8100/docs/design/20260912-0022-full-sa-plan-integration-claude-fable-plan.md)

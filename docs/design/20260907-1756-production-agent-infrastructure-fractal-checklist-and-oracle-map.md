...
LAW-VFS-05 received an observed predicate and LAW-VFS-08 was found NOT IMPLEMENTED in ZigVM (no path jail) by `t3-vfs-predicates`; the VFS wiki and ADR-046 records marking it PASS are nonconformant. `selfcheck-sa-plan` now executes all 13 sa-plan suites and binds each row to exit code and observed law count (`t5-saplan-selfcheck-exec`). `t7-wave-and-mirage-gates` then found two gates worse than existence-only: `selfcheck-wave3-cycles` and `selfcheck-wave4-cycles` printed 30 PASS lines with **zero predicates**; both are now `[INVENTORY]` rows bound to a ratification record per EV-55..EV-84 and fail when a record is absent (negative control: 2/15, exit 1). The four `selfcheck-mirage*` gates now execute their Hermes suites and reproduce each suite's own verdict line, which states that unikernel deployment remains NOT_VERIFIED. Still existence-only (recorded, not repaired): `selfcheck-wave3-cycles`, `selfcheck-wave4-cycles`, `selfcheck-omni-matrix`, `selfcheck-hermes-bionic`, `selfcheck-vertical-slice`, `selfcheck-zigvm-add`, `selfcheck-raga`, the three `selfcheck-mirage*` commands and `selfcheck-forecast`. Their `100% Green` summaries mean "required files present".
...
3. **Sandbox isolation**: egress allow-list enforced at Podman network or Solo5 tender; in-repo seccomp profile; replace `simulate_*` with tender execution. The ZigVM path jail (LAW-VFS-08) has an executable specification in `hermes_vfs_oracle` but is **blocked on toolchain**: no Zig compiler exists on this host, so jail code cannot be built or tested here.

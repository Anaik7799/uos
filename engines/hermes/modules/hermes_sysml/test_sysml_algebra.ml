...
let test_id_left =
  Test.make ~name:"id_left" arb_transition
    (fun t -> compose id t = t)

let test_id_right =
  Test.make ~name:"id_right" arb_transition
    (fun t -> compose t id = t)

let test_assoc =
  Test.make ~name:"assoc" (triple arb_transition arb_transition arb_transition)
    (fun (f, g, h) ->
...
let () =
  ...
  let self = Suite_telemetry.observe ~suite:"test_sysml_algebra" ~passed:(if code = 0 then 1 else 0) ~failed:code ~skipped:0 in

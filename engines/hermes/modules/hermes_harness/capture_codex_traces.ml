(* Capture pinned CODEX-RUNTIME traces for model_routing.codex_runtime. Offline:
   the frozen shaping trio is pure. The captured fixture, not the probe's
   hand-derived expectations, is the proof. *)
      ...
      | Ok reference_revision ->
          ...
          List.iter
            (fun (scenario_id, params_json) ->
              ...
              in
              match
                Reference_capture.capture ~adapter_basename:"codex_message_shapes_adapter.py" ~root
          ...
          Printf.printf "\ncaptured %d/%d codex scenarios\n"
            (List.length Parity_compare.codex_scenarios - !failures)

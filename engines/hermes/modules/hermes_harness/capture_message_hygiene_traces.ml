(* Capture pinned MESSAGE-HYGIENE traces for agent_loop.message_hygiene.
   Offline: the frozen strict-API tool_call sanitizer is a pure static method.
   The labeled cases are carried in params so the existing capture path drives
   a different adapter unchanged. Deliberate and explicit -- it writes
   evidence. *)
      ...
      | Ok reference_revision ->
          ...
          List.iter
            (fun (scenario_id, params_json) ->
              ...
              in
              match
                Reference_capture.capture ~adapter_basename:"message_hygiene_adapter.py" ~root
          ...
          Printf.printf "\ncaptured %d/%d hygiene scenarios\n"
            (List.length Parity_compare.hygiene_scenarios - !failures)
            (List.length Parity_compare.hygiene_scenarios);

let () =
  let path = Filename.temp_file "hermes-plan" ".sqlite3" in
  Fun.protect
    ~finally:(fun () -> if Sys.file_exists path then Sys.remove path)
    (fun () ->
      let features = [ "agent_loop"; "tool_execution" ] in
      (* Topological order: a dependency is always materialized before its
         dependent, so the store can resolve the reference. *)
      let capabilities =
        [ ("tool_execution.tool_registry", []);
          ...
          ("agent_loop.conversation_loop", [ "tool_execution.tool_dispatch" ]) ]
      in
      match Hermes_plan.materialize ~path ~snapshot_digest:"snapshot" ~features ~capabilities with
      | Error message -> failwith message
      | Ok plan ->
          assert (plan.plan_id = "hermes/snapshot");
          assert (plan.task_ids = [ "feature/agent_loop"; "feature/tool_execution" ]);
          assert
            (plan.capability_task_ids
            = [ "capability/tool_execution.tool_registry";
                "capability/tool_execution.approval_policy";
                "capability/tool_execution.tool_dispatch";
                "capability/agent_loop.conversation_loop" ]);
          (* Replaying the same materialization is idempotent. *)
          (match
             Hermes_plan.materialize ~path ~snapshot_digest:"snapshot" ~features ~capabilities
           with
          | Error message -> failwith message
          | Ok replay -> assert (replay = plan));
          match Hermes_plan.summary ~path ~plan_id:plan.plan_id with
          | Error message -> failwith message
          | Ok (total, _ready, executing, completed) ->
              assert (total = List.length features + List.length capabilities);
              assert (executing = 0);
              assert (completed = 0))

let () =
  let self =
    Suite_telemetry.observe ~suite:"test_hermes_plan" ~passed:1 ~failed:0 ~skipped:0
  in

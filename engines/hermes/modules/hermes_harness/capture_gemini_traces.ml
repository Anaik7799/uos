(* Capture pinned GEMINI-ADAPTER traces (first cut: schema sanitizer) for
   model_routing.gemini_adapter. Offline: the frozen sanitizer is pure (math
   only, no httpx). The captured fixture is the proof. *)
      ...
      | Ok reference_revision ->
          ...
          List.iter
            (fun (scenario_id, params_json) ->
              ...
              in
              match
                Reference_capture.capture ~adapter_basename:"gemini_schema_adapter.py" ~root
          ...
          Printf.printf "\ncaptured %d/%d gemini scenarios\n"
            (List.length Parity_compare.gemini_scenarios - !failures)

(* Capture pinned ANTHROPIC-ADAPTER traces for model_routing.anthropic_adapter.
   Offline: the frozen shaping trio is pure (no clock/env/net/fs). The captured
      ...
      | Ok reference_revision ->
          ...
          List.iter
            (fun (scenario_id, params_json) ->
              ...
              in
              match
                Reference_capture.capture ~adapter_basename:"anthropic_adapter_adapter.py" ~root
          ...
          Printf.printf "\ncaptured %d/%d anthropic scenarios\n"
            (List.length Parity_compare.anthropic_scenarios - !failures)

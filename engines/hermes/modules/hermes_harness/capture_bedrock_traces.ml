(* Capture pinned BEDROCK-CONVERSE traces for model_routing.cloud_vendor_adapters.
   Offline: the frozen build_converse_kwargs is pure (import neutralized against
   network installs by the adapter). The captured fixture is the proof. *)
      ...
      | Ok reference_revision ->
          ...
          List.iter
            (fun (scenario_id, params_json) ->
              ...
              in
              match
                Reference_capture.capture ~adapter_basename:"bedrock_converse_adapter.py" ~root
          ...
          Printf.printf "\ncaptured %d/%d bedrock scenarios\n"
            (List.length Parity_compare.bedrock_scenarios - !failures)

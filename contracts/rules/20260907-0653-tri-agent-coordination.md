...
14. **SYNC-14 Defense survivability & offline autonomous degradation.** When network access to remote frontier models (Claude, Codex, AGY, OpenRouter) degrades, terminates, or is denied (tactical electronic warfare, communications blackout, subsystem crashes), the system MUST degrade gracefully without stall. Operational autonomy degrades to Tier 2 (ZigVM-supervised local SLM inference on bare-metal Modular MAX/Mojo execution fabric) and Tier 1 (Pure Gleam/OTP 29 supervisors and Hermes OCaml Rete-UL forward chaining). Quorum falls back from 3-agent remote consensus to the local tri-authority: (1) Gleam/OTP 29 supervisor, (2) Hermes Gospel/Z3/Rete-UL formal oracle, and (3) ZigVM deterministic execution engine. Fail-closed preflight gating (`tools/preflight`) remains strictly enforced; all local actions and evidence are committed to standalone Jujutsu (`.jj/`) and reconciled via the coordinator upon network restoration.

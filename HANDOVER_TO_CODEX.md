...
The operator now requires full DMC/TCM, denotational intent, an L0–L9 algebraic atlas, F Prime based Gleam actors, SysML implementation, and an ecology of communicating actors informed by ZigVM and Harness-Bionic's FPP/Zenoh code. The latest directive requires C3I's Zenoh NIF and Zenoh for all UOS application/domain communication; compare every located Gleam/Elixir integration for verified feature coverage and measured performance, then implement the complete pinned Zenoh feature surface and actual call paths. The addendum compares three native candidates and identifies correctness and benchmark gaps; no fastest or complete integration is claimed. Original external OCaml remains unchanged. The added documents retain the complete web/wiki/ZK/KM test inventory, research, compiler/solver and four-cycle browser requirements.

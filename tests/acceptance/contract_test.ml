...
let () =
  ...
  check "NaN rejected"
    (Result.is_error (parse_json_strict "NaN")
     && Result.is_error (first_duplicate_or_nested "$" (`Float nan)));
  check "infinity rejected"
    (Result.is_error (parse_json_strict "Infinity")
     && Result.is_error (first_duplicate_or_nested "$" (`Float infinity)));
  let too_deep =
    String.make (maximum_json_depth + 1) '[' ^ "0"
    ^ String.make (maximum_json_depth + 1) ']'
  in
  check "deep JSON rejected before parse" (Result.is_error (parse_json_strict too_deep));
  check "wide array rejected"
    (Result.is_error
      (first_duplicate_or_nested "$"
        (`List (List.init (maximum_array_items + 1) (fun _ -> `Null)))));
  check "wide object rejected"
    (Result.is_error
      (first_duplicate_or_nested "$"
        (`Assoc
          (List.init (maximum_object_fields + 1)
            (fun index -> (string_of_int index, `Null))))));
  let extra_fixture =
  ...
  let observation =
    { operation_supported = true; adapter_registered = false; built = true; executed = true;
      passed = false; exit_code = 2; status = Status_error; passing_tests = 0; children_reaped = true;
      timed_out = false; stdout_overflow = false; stderr_overflow = false; stdout = "";
      stderr = "missing"; duration_ms = 1.; data = None; applied_limits = no_process_limits }
  in
  let receipt =
    { receipt_id = "r"; task_id = "E02"; case_id = "E02-regression"; operation = "runner.verify";
      ...
      sources =
        List.init 5 (fun index ->
          { binding_path = Printf.sprintf "tests/acceptance/input-%d.ml" index;
            binding_sha256 = String.make 64 'e' });
      ...
      assertion_duration_ms = 0.1; receipt_write_calibration_ms = 0.2 }
  in
